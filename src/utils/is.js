...
function $number(x) {
  if (arguments.length > 1 && arguments[1]) {
    return typeof x === 'number' || /^(\-?)(\.?|[0-9]+\.?)[0-9]+$/gmi.test(x);
  } else {
...
function $array(x) {
  return Object.prototype.toString
    .call(x) === "[object Array]" || Array.isArray(x);
}
...
function $port(x) {
  return (is.string(x) || is.number(x)) && ((x) > 0 && (x) < 65535);
}

function $between(min, max, val) {
  if (is.number(min) && is.number(max) && is.number(val)) {
    return val >= min && val <= max;
  }

  if (is.string(min) && is.string(max) && is.string(val)) {
    return val >= min && val <= max;
  }

  if (is.date.valid(min) && is.date.valid(max) && is.date.valid(val)) {
    return val >= min && val <= max;
  }
...
function $date(x) {
  ...
  if (x && !isNaN(new Date(x).getDate())) {
    return true;
  }
  if (x && typeof x.getMonth === 'function') {
    return true;
  }
  return false;
}
...
function $type(x) {
  // The order matters because in "date" type it checks also string
  // so it is used before the "string" type to avoid inconvients.
  if ($number(x)) return 'number';
  if ($boolean(x)) return 'boolean';
