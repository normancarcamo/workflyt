  ...
  BEGIN
    ...
    FOREACH ELEMENTOS SLICE 2 IN ARRAY $1 LOOP
      ...
      FOREACH ELEMENTO SLICE 1 IN ARRAY ELEMENTOS LOOP
        ...
        FN_NAME  = SCH_NAME || '.' || TBL_NAME || '_gencode'; -- example: 'nz.person_gencode'
        SEQ_NAME = SCH_NAME || '.' || TBL_NAME || '_seq'; -- example: 'nz.person_seq'
        -- FUNCTION:
        PERFORM (SELECT FORMAT('DROP FUNCTION IF EXISTS %s() CASCADE', FN_NAME));
        PERFORM (SELECT FORMAT('CREATE FUNCTION %s() RETURNS TRIGGER AS $FN$ BEGIN NEW.code = %s; RETURN NEW; END; $FN$ LANGUAGE plpgsql', FN_NAME, PREFIX || NEXTVAL(SEQ_NAME)));
        -- TRIGGER:
        PERFORM (SELECT FORMAT('DROP TRIGGER IF EXISTS %s ON %s CASCADE', FN_NAME, TBL_NAME));
        PERFORM (SELECT FORMAT('CREATE TRIGGER IF NOT EXISTS %s BEFORE INSERT ON %s FOR EACH ROW EXECUTE PROCEDURE %s()', FN_NAME, TBL_NAME, FN_NAME));
      END LOOP LOOP_CHILD;
