...
export const getSuppliers = new Datalizer({
  query: schema.QUERY({
...
export const getSupplier = new Datalizer({
  ...
  query: schema.QUERY({
    attributes: schema.ATTRIBUTES(supplier.attributes),
    include: schema.INCLUDE(supplier.associations),
    paranoid: schema.BOOLEAN({ $optional: true })
  }, { $max: 10 })
});

export const updateSupplier = new Datalizer({
  params: schema.PARAMS(
    { supplier: schema.UUID() },
    { $length: 1 }
  ),
...
export const deleteSupplier = new Datalizer({
  params: schema.PARAMS(
    { supplier: schema.UUID() },
    { $length: 1 }
  ),
...
export const getMaterial = new Datalizer({
  params: schema.PARAMS(
    { supplier: schema.UUID(), material: schema.UUID() },
    { $length: 2 }
  ),
  query: schema.QUERY({
    attributes: schema.ATTRIBUTES(material.attributes),
